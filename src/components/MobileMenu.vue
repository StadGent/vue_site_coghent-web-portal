  ...
  <div v-show="menuIsVisible" class="z-50 fixed top-0 left-0 flex flex-row min-h-screen w-screen">
    <div class="w-9/12 flex flex-col w-full">
      ...
      <div class="flex justify-between flex-col h-full bg-background-medium pl-6">
        <ul>
          <li v-for="menuItem of menuItems" :key="menuItem.id" class="flex items-center justify-left border-b-2 border-background-light h-16 cursor-pointer" @click="() => openMenuItem(menuItem)">
            {{ menuItem.title }}
          </li>
        </ul>
        <ul class="border-t-2 border-background-light flex object-bottom">
          <li v-for="menuItem of menuItemsBottom" :key="menuItem.id" class="flex items-center justify-left h-16 cursor-pointer" @click="() => openMenuItem(menuItem)">
            {{ menuItem.title }}
          </li>
        </ul>
      </div>
