<template>
  <section>
    <p style="font-size:25pt"><strong>{{ title }}</strong></p>
    <div v-for="image in mediafiles" :key="image">
      <img :src="image" style="width:500px; height:500px; margin:auto"/>
    </div>
    <p v-for="type in types" :key=type><strong>{{type}}</strong></p>
    <p style="max-width:500px; margin:auto">{{description}}</p>
  </section>
</template>
...
import { GetEntityByIdDocument, MetaKey } from '../../coghent-vue-3-component-library/src/queries'
