...
const useGraphqlErrors = (_errorResponse: ErrorResponse) => {
  ...
  const invalidToken = (error: GraphQLError) => {
    ...
    checks.push(error.extensions?.response && error.extensions?.response.body.message && error.extensions?.response.body.message === 'invalid_token: The access token provided is expired, revoked, malformed, or invalid for other reasons.')
    return checks.some(check => check === true)
  }
